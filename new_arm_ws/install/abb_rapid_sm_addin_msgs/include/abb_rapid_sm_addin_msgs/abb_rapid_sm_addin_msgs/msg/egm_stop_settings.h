// generated from rosidl_generator_c/resource/idl.h.em
// with input from abb_rapid_sm_addin_msgs:msg/EGMStopSettings.idl
// generated code does not contain a copyright notice

#ifndef ABB_RAPID_SM_ADDIN_MSGS__MSG__EGM_STOP_SETTINGS_H_
#define ABB_RAPID_SM_ADDIN_MSGS__MSG__EGM_STOP_SETTINGS_H_

#include "abb_rapid_sm_addin_msgs/msg/detail/egm_stop_settings__struct.h"
#include "abb_rapid_sm_addin_msgs/msg/detail/egm_stop_settings__functions.h"
#include "abb_rapid_sm_addin_msgs/msg/detail/egm_stop_settings__type_support.h"

#endif  // ABB_RAPID_SM_ADDIN_MSGS__MSG__EGM_STOP_SETTINGS_H_
